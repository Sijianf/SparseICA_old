...
my_sparseICA = sparseICA(xData = xmat, n.comp = 3,U.list=NULL,whiten = "eigenvec", restarts.pbyd = 10,lambda = sqrt(2)/2, nu = best_nu,eps = 1e-6,maxit.laplace = 500, converge_plot = T)
par(mfrow=c(1,3))
...
my_PCA_sparseICA = PCA_sparseICA(xData = xmat, n.comp = 3,U.list=NULL,whiten = "eigenvec", restarts.pbyd = 10, lambda = sqrt(2)/2, nu = best_nu,eps = 1e-6,maxit.laplace = 500, converge_plot = T)
par(mfrow=c(1,3))
