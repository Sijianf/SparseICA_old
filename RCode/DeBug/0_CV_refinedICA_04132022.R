  ...
  if(CV_method=="projection"){
    ...
    for (j in 1:K) {
      ...
      for (i in 1:length(nu_list)) {
        nu = nu_list[i]
        V_negK_relax_laplace = refinedICA(xData = X_negk, n.comp = Q, whiten = "none", restarts.pbyd = restarts.pbyd, method = method, lambda = lambda, nu = nu, eps = eps,loop_laplace = loop_laplace, loop_logistic = loop_laplace, show_message=show_message,converge_plot = F)
