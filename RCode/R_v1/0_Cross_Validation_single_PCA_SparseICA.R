##########################################################################################
# Cross-Validation for the Tuning Parameter nu in single-subject PCA+sparseICA practice ##
###################### Zihang Wang ####### Last update: 9/25/2022 ########################
##########################################################################################

CV_PCA_sparseICA = function(xData,n.comp,fold,CV_method=c("projection","fastICA_logistic","fastICA_tanh"),restarts.pbyd = 10, method = c("sparse_laplace","sparse_logistic"), lambda = sqrt(2)/2, eps = 1e-5,loop_laplace = 500, loop_logistic = 500,nu_list=c(0.001,0.005,0.01,0.03,0.05,0.07,0.1,0.3,0.5,0.7,1,1.5,2),random_seed=2022,show_message=T,make_plot=T){
  start_time = Sys.time()
  ...
  if(CV_method=="projection"){
    ...
    for (j in 1:K) {
      ...
      # scale X_negk prior to PCA
      # X_negk = scale(X_negk)
      
      for (i in 1:length(nu_list)) {
        nu = nu_list[i]
        # do PCA first
        subj_PCA=prcomp_irlba(X_negk,Q,center = T,scale. = T)
        PC_negk=subj_PCA$x
        dimnames(PC_negk)=NULL
        
        V_negK_relax_laplace = sparseICA(xData = PC_negk, n.comp = Q,whiten = "eigenvec", restarts.pbyd = restarts.pbyd, method = method, lambda = lambda, nu = nu, eps = eps,loop_laplace = loop_laplace, loop_logistic = loop_laplace, show_message=show_message,converge_plot = F)
        V_negk = V_negK_relax_laplace$estV
        V_negk = cbind(rep(1,nrow(V_negk)),V_negk)
        temp_mat = solve(crossprod(V_negk), t(V_negk))%*%X_k
        CV_score[j,i] = sum((X_k-V_negk%*%temp_mat)^2)
      }
    }
