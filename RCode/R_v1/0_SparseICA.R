...
############################### Last Updates: 11/5/2022 ##################################
##########################################################################################

##########################################################################################
# Updates information:
# Updated 2022.05.10 for add U.list as user-specified initial values for U, change the names of return values V->S, U->M
# Updated 2022.05.10 for change the function name, method names, information displayed, delete W.list from the arguments
...
sparseICA = function(xData,n.comp,nu = 1,U.list=NULL,whiten = c('eigenvec','sqrtprec','none'), orth.method=c('svd','givens'), restarts.pbyd = 10, method = c( "sparse_laplace","sparse_logistic", "fast_logistic", "fast_tanh"), lambda = sqrt(2)/2, irlba = FALSE, eps = 1e-06, maxit.laplace = 500, maxit.logistic = 500, show_message=T, converge_plot = F){
  ...
  if (method == "sparse_logistic") {
    ...
    if(show_message){
      cat("MESSAGE: The algorithm for relax logistic does not converge (<",eps,")in",maxit.logistic,"iterations for",m,"/",runs,"different start values!\n")
      #cat("Consider increasing the number of loop_laplase.\n")
    }
