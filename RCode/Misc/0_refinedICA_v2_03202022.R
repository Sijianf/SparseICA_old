...
##########################################################################################
# Updated 2022.03.20 for optimization function
...
refinedICA = function(xData = xmat, n.comp = 3, W.list = NULL, whiten = c('eigenvec','sqrtprec','none'), orth.method=c('svd','givens'), restarts.pbyd = 1, method = c( "relax_laplace","relax_logistic", "FastICA_logistic", "FastICA_tanh"), lambda = sqrt(2)/2, nu = 1, irlba = FALSE, eps = 1e-06, loop_laplase = 500, loop_logistic = 500, converge_plot = F){
  ...
  if (method == "relax_laplace") {
    ...
    cat("The algorithm for relax laplase does not converge in",loop_laplase," iterations for ",m,"/",runs," different start values!\n")
  ...
  if (method == "relax_logistic") {
    ...
    cat("The algorithm for relax laplase does not converge in",loop_logistic," iterations for ",m,"/",runs," different start values!\n")
    cat("Consider increasing the number of loop_laplase.\n")
