...
# Updated 2022.03.24 for changing algorithm ending criteria: converage < eps
# Updated 2022.03.20 for optimization function, adding warnings,adding running time
...
refinedICA = function(xData = xmat, n.comp = 3, W.list = NULL, whiten = c('eigenvec','sqrtprec','none'), orth.method=c('svd','givens'), restarts.pbyd = 10, method = c( "relax_laplace","relax_logistic", "FastICA_logistic", "FastICA_tanh"), lambda = sqrt(2)/2, nu = 0.1, irlba = FALSE, eps = 1e-05, converge_plot = F){
  ...
  if (method == "relax_laplace") {
    ...
    for (k in 1:runs) {  # make a list if restarts.pbyd > 1, we should update the index of W.list if restarts.pbyd > 1
      ...
      while (my_converge > eps) { 
        ...
        my_converge = mean((lagV-newV)^2)/ncol(lagV)
        converge[iteration] = my_converge # frobICA(S1=newV, S2=lagV) # BRISK note: normalize by number of columns
        lagV = newV
      } 
      ...
      cat("The relax laplace algorithm converges (<",eps,") in",iteration,"iterations in #",k,"starting value run.\n")
  ...
  if (method == "relax_logistic") {
    ...
    for (k in 1:runs) { # Make a list if restarts.pbyd > 1
      ...
      if (k != 1) {out.list[[k]] = out.list[[1]]}
      out.list[[k]]$loglik = sum(-newV/(sqrt(3)/pi) - 2*log(1+exp(-newV/(sqrt(3)/pi)))) - 1/(2*nu)*(norm(newV-xData%*%newU,type = "F"))^2
      loglik[k] = out.list[[k]]$loglik
      out.list[[k]]$estV = newV
      out.list[[k]]$estU = newU
      out.list[[k]]$xData = xData
      ...
      cat("The relax logistic algorithm converges (<",eps,") in",iteration,"iterations in #",k,"starting value run.\n")
    } 
    # End the loop to make a list
