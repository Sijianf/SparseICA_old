...
sparseICA = function(xData, n.comp , whiten = c('eigenvec','sqrtprec','none'), orth.method=c('svd','givens'), restarts.pbyd = 10, method = c( "sparse_laplace","sparse_logistic", "fast_logistic", "fast_tanh"), lambda = sqrt(2)/2, nu = 0.1, irlba = FALSE, eps = 1e-06, loop_laplace = 500, loop_logistic = 500, show_message=T, converge_plot = F){
  ...
  if (method == "sparse_laplace") {
    ...
    if(show_message){
      cat("MESSAGE: The algorithm for relax laplase does not converge (<",eps,") in",loop_laplace,"iterations for",m,"/",runs,"different start values!\n")
  ...
  if (method == "sparse_logistic") {
    ...
    if(show_message){
      cat("MESSAGE: The algorithm for relax logistic does not converge (<",eps,")in",loop_logistic,"iterations for",m,"/",runs,"different start values!\n")
      #cat("Consider increasing the number of loop_laplase.\n")
    }
