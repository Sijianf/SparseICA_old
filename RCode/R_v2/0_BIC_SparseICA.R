...
BIC_sparseICA = function(xData,n.comp,nu_list = seq(0.1,4,0.01),U.list=NULL,whiten = c('eigenvec','sqrtprec','none'), orth.method=c('svd','givens'), restarts.pbyd = 10, method = c( "sparse_laplace","sparse_logistic", "fast_logistic", "fast_tanh"), lambda = sqrt(2)/2, irlba = FALSE, eps = 1e-06, maxit.laplace = 500, maxit.logistic = 500, show_message=T, BIC_plot = F){
  ...
  ref_sparseICA = sparseICA(xData = xmat, n.comp = n.comp,whiten = whiten, restarts.pbyd = restarts.pbyd, method = method, lambda = sqrt(2)/2, nu = best_nu,eps = 1e-6,maxit.laplace = 500, maxit.logistic = 500,show_message = show_message, converge_plot = F)
  ...
  for (i in 1:length(nu_list)) {
    my_nu = nu_list[i]
    temp_sparseICA = sparseICA(xData = xmat,n.comp = n.comp,U.list = my_U_list,whiten = whiten, restarts.pbyd = restarts.pbyd, method = method, lambda = sqrt(2)/2, nu = my_nu,eps = 1e-6,maxit.laplace = 500, maxit.logistic = 500,show_message = show_message, converge_plot = F)
